...
public class Main {
	...
	static int arr[], tree_min[], tree_max[];
	static int a, b, min, max;
	...
	public static void main(String[] args) throws Exception {
		...
		cnt = 2 << height;
		tree_min = new int[cnt];
		tree_max = new int[cnt];
	...
	private static void print(int node, int start, int end) {
		...
		if (start == end || a <= start && end <= b) { // 리프 노드에 도달, 현재 구간이 b-c 사이에 존재
			min = Math.min(min, tree_min[node]);
			max = Math.max(max, tree_max[node]);
			return;
		}
		int mid = (start + end) / 2;
